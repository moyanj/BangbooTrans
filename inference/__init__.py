...
class Inference:
    def __init__(self, model_name,model_base='models', compile_model=False, force_cpu=False):
        ...
        self.input_lang = dataset.Lang.load(open(os.path.join(model_path,'input_vocab.json')), self.device)
    ...
    def eval_steam(self, sentence, max_length=200):
        with torch.no_grad():
            ...
            decoder_input = torch.tensor([[dataset.SOS_token]], device=self.device)  # SOS
            ...
            for di in range(max_length):
                ...
                if topi.item() == dataset.EOS_token:
                    ...
                    decoded_char = self.output_lang.index2char.get(str(topi.item()), 'UNK')
